...
public:
  ...
  double x[NDIM], x1[NDIM], u[NDIM], phi, 
    phi1, *dpot_dx, *dpot_dx_filt;
  ext_node();
  ~ext_node();
};

//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---: 
ext_node::ext_node() : dpot_dx(NULL), dpot_dx_filt(NULL) { 
  // printf(" in ext_node() NDIM %d, n_coef %d\n",NDIM,n_coef);
...
void coupling_inv_hook::init(Mesh &mesh,Dofmap &dofmap,
  ...
  assert(Uinf!=0.);
  TGETOPTDEF_ND(options,int,n_coef,0);
  assert(n_coef>=1);
...
void coupling_inv_hook::time_step_post(double time,int step,
  ...
  FastMat2 x(2,npoint,NDIM), xi(2,npoint,NDIM), psi(2,npoint,nterm), 
    ...
    pot(1,npoint),dpot_dxi(1,NDIM), dpot_dx(2,n_coef,NDIM), 
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---: 
void coupling_inv_hook::close() {
  for (int j=0; j<ext_node_data.size(); j++) {
    delete ext_node_data[j];
  }
  ext_node_data.clear();
}
