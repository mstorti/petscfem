//__INSERT_LICENSE__
//$Id: inviscid.cpp,v 1.18.2.1 2003/01/11 13:56:26 mstorti Exp $
#define _GNU_SOURCE
...
void coupling_inv_hook::time_step_post(double time,int step,
  ...
  // filter coefficients 
#if 1
  double omega=0.25, xif=1;
  // double omega=0.5, xif=2;
  double a_coef[] = {1./square(omega)+xif/omega+0.5, 
		     -2/square(omega),
		     1./square(omega)-xif/omega+0.5};
  double b_coef[] = {0.5+xif/omega,0.,0.5-xif/omega};
#elif 0
  double omega=0.25, xif=2;
  // double omega=0.5, xif=2;
  double a_coef[] = {1./square(omega)+xif/omega+0.5, 
		     -2/square(omega),
		     1./square(omega)-xif/omega+0.5,0.};
  double bb_coef[] = {0.5+xif/omega,0.,0.5-xif/omega};
  double b_coef[4];
  for (int j=0; j<4; j++) b_coef[j] = 0.;
  for (int j=0; j<3; j++) {
    b_coef[j]   += 2*bb_coef[j];
    b_coef[j+1] -= bb_coef[j];
  }
#else
  omega=0.;
