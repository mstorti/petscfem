//__INSERT_LICENSE__
//$Id: alehook.cpp,v 1.1 2003/03/19 17:24:52 mstorti Exp $
#define _GNU_SOURCE
...
void ale_hook::time_step_post(double time,int step,
  ...
  PetscPrintf(PETSC_COMM_WORLD,"ALEHOOK: begin time_step_post ....\n");
  char * const argv[] = {"/usr/bin/make","spillway_mesh",NULL};
  int stat = execv(argv[0],argv);
  if (stat==-1) {
    printf("ALEHOOK: Couldn't \"execv\" octave process...\n");
    abort();
  }
  FILE *fid = fopen("spiller.nod.tmp","w");
...
#define NODEDATA(j,k) VEC2(mesh->nodedata->nodedata,j,k,nu)
  for (int j=0; j<nnod; j++) {
    for (int k=0; k<ndim; k++) {
      int nread = fscanf(fid,"%f",&NODEDATA(j,k));
      assert(nread==1);
    }
    double dummy;
