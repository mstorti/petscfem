...
PETSc-FEM in the \verb+tools/petscfem.el+ Emacs Lisp file. In order to
use the mode, you have to copy the file  \verb+tools/petscfem.el+ to
somewhere accessible for Emacs (\verb+/usr/share/emacs/site-lisp+  is
a good candidate). There is also a file \verb+tools/petscfem-init.el+
that contains some basic configuration, you can also copy it, or
insert directly its contents into your \verb+.emacs+. 
%
\begin{verbatim}
;; Load basic PETSc-FEM mode
(load-file "/path/to/tools/petscfem.el")

;; Load `petscfem-init' or insert directly its contents
;; below and configure
(load-file "/path/to/tools/petscfem-init.el")
\end{verbatim}
%
...
(\verb+tools/options.info+) that has a page for each of the
