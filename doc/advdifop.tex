...
\begin{itemize}
%
\item{\tt null}: (\verb+var_len=0+) This means $R\equiv 0$. No
...
\delta_{\mu\nu}\). 
%
\item{\tt scalar\_per\_field} {\tt [var\_len==ndof*ndof]}] 
A scalar for each field: 
\[  R_{\mu\nu} = {\bar R_\mu} \, \delta_{\mu\nu}\ \ \ 
                  \textrm{No sum over \(\mu\)} \]
%
\item{\tt full} {\tt [var\_len==ndof*ndof]}] General case,
stored per row
\(\{\{R_{\mu\nu},\nu=1,\ndof\},\mu=1,\ndof\}\)
\end{itemize}
...
\begin{itemize}
%
\item{\tt null}: (\verb+var_len=0+) This means $S\equiv 0$. No
constant source term is present. 
%
\item {\tt global\_scalar} {\tt [var\_len==1]} (A scalar) 
The same source in all fields \(S_{\mu} = {\bar S}\). 
%
\item{\tt full} {\tt [var\_len==ndof]}] General case. One source term
value per field. 
\end{itemize}
