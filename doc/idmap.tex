...
permutation. Assume that the $n\times n$ matrix $Q$ is a permutation
matrix, so that if $!y$, $!x$ are vectors of dimension $n$ and
%
\begin{equation} 
   !y = !Q \, !x
\end{equation}
...
\end{equation}
%
where $P$ is the permutation associated with $Q$. For instance if
...
iident[n]}$, so that both $Q$ and $Q\muno$ can be stored at the cost
of  $n$ integer values for each. Also, both multiplication and
...
$u$, $v$ and $p$).  In a first description, we may arrange the vector
of unknowns as 
%
\begin{equation} 
   !U_\NF = \begin{sqmat}{c}
...
the ``node/field'' (this accounts for the $NF$ subindex) description
...
\end{equation}
%
where $!U$ is the ``reduced'' array of unknowns, $\barU$ 
representing the externally fixed values, and  $Q$, $Q^*$ appropriated
...
\end{align}
%
so that the corresponding file in $!Q$ is null and the file in $\barQ$
...
values while leaving the outgoing components free. If $!w=[u,v,p]$ is
the state vector of the fluid at some node $j$ on the outlet boundary
(see figure~\ref{fg:idmap}), then $!V!w = !u$ are the
eigen-components, where $!V$ is the change of basis matrix.
...
\end{align}
%
And the relation defining matrices $!Q$ and $\barQ$ are
%
\begin{align}
...
\end{align}
%
As we can see the boundary conditions result in sparse $!Q$ and
$\barQ$ matrices. Moreover, in real (large) problems most of the rows
correspond to interior nodes (such as node 5 here) so that $!Q$ is
very close to a permutation matrix. If we think at accessing the
elements of $!Q$ by row, then could store $!Q$ as a sparse matrix, but
...
i.e. given $!U_\NF$ and $\barU$ to find $!U$, for instance when 
initializing a temporal problem. Of course, this may not be possible
for arbitrary  $!U_\NF$ and $\barU$, since $!Q$ is in general
...
The class of matrices representing $!Q$ and $\barQ$ should have the
...
the pointer in $row_map$ and this returns the desired row. Similar
...
When solving a linear (\ref{eq:urep}) for $!U$, it is a key point to
take into account that most part of $!Q$ are independent blocks, so
...
\end{align}
%
So that $!Q$ decomposes in three blocks of $3\times 2$, $1\times 1$,
