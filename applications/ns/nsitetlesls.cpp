...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---: 
#undef __FUNC__
#define __FUNC__ "nsi_tet_les_ls::ask(char *,int &)"
int nsi_tet_les_ls::
...
#undef __FUNC__
#define __FUNC__ "nsi_tet_les_ls::assemble"
int nsi_tet_les_ls::
...
#define NN_IDX(j) ELEMIPROPS_ADD(j,0)
  ...
  int nH = nu-ndim;
  // anterior:  FMatrix  Hloc(nel,nH),vloc_mesh(nel,ndim),v_mesh(ndim);
  FMatrix  Hloc(nel,nH),vloc_mesh(nel,ndim),v_mesh(ndim);
  FastMat2  philoc(2,nel,nH);
...
#define WAS_SET arg_data_v[ja_hmin].was_set
  ...
  //o Flag to *turn on* level set computation
  SGETOPTDEF(int,LEVEL_SET_flag,0);
  ...
  //o Densities for level set
  TGETOPTDEF(thash,double,rho_l,1.);
  TGETOPTDEF(thash,double,rho_g,1.);
...
#define VISC (*(propel+viscosity_indx))
  ...
  for (int k=el_start; k<=el_last; k++) {
    ...
    for (kloc=0; kloc<nel; kloc++) {
      ...
      if(nH>0) { // por ahora, si hay ale no hay level set
	if (ALE_flag){
	  Hloc.ir(1,kloc+1).set(&NODEDATA(node-1,0)+ndim);
	}
	else if (LEVEL_SET_flag){
	  philoc.ir(1,kloc+1).set(&NODEDATA(node-1,0)+ndim);
	} else {
	}
      }
    }
...
// #define WPG_SUM  (gp_data.wpg_sum)
	...
	// printf("initial viscosity %f ",nu_eff);
	if (LEVEL_SET_flag){
	  nu_eff = double(v02.prod(SHAPE,v01,-1,-1));
	  if ( nu_eff>nu_M ) nu_eff = nu_M;
	  if ( nu_eff<nu_m ) nu_eff = nu_m;
	  if (0) {
	    printf("viscosity %f \n",nu_eff);
	  }
