//__INSERT_LICENSE__
//$Id: nsi_tet.cpp,v 1.10 2001/05/30 18:21:50 mstorti Exp $
...
#define VISC (*(propel+viscosity_indx))
  ...
  FastMat dshapex,dshapext,Jaco(ndim,ndim),iJaco(ndim,ndim),
    grad_u(ndim,ndim),grad_u_star,grad_u_star_t,resmom(nel,ndim),dresmom,
...
//	tau_supg = psi*h_supg/(2.*velmod);
	...
	tau_pspg = h_pspg*h_pspg/ 2 / ( 6*VISC + sqrt(u2)*h_pspg ) ;
	...
	if (weak_form) {
	  ...
	  resmom -= wpgdet * dshapex.t() *
	    (VISC * grad_u_star - p_star * eye);	
#else
	  tmp1.set(grad_u_star);
	  tmp1.scale(VISC);
...
#ifndef USE_FASTMAT
	  resmom -= wpgdet * dshapex.t() * (VISC * grad_u_star );	
	  resmom -= wpgdet * SHAPE.t() * grad_p_star.t() ;	
#else
	  // tmp1.set(grad_u_star);
	  FMp(tmp1,dshapext,grad_u_star);
	  FMaxpy(resmom,-wpgdet*VISC,tmp1);
	...
	// Parte difusiva
#ifndef USE_FASTMAT
	matlocmom += VISC * dshapex.t() * dshapex ; // grad_u_star;
#else
	FMp(tmp7,dshapext,dshapex);
	FMaxpy(matlocmom,VISC,tmp7);
#endif
