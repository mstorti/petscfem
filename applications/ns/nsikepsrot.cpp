//__INSERT_LICENSE__
/* $Id: nsikepsrot.cpp,v 1.35.10.1 2007/02/19 20:23:56 mstorti Exp $ */
...
// modif nsi_tet
#undef __FUNC__
#define __FUNC__ "nsi_tet_keps_rot::assemble"
int nsi_tet_keps_rot::assemble(arg_data_list &arg_data_v,Nodedata *nodedata,
...
#define JDOFLOC(j,k) VEC2(jdofloc,j,k,ndof)
  ...
  int non_inertial_force_was_added=0;
...
#define VISC (*(propel+viscosity_indx))
  ...
  //GPdata gp_data(geom,ndim,nelr,npg);
  ...
  FMatrix dshapex,dshapext,Jaco(ndim,ndim),iJaco(ndim,ndim),
    ...
    dresmom(nelr,ndim),matij(ndof,ndof),P_pspg,svec;

  FMatrix grad_p_star(ndim),u,u_star,du,
    rescont(nelr),dmatu(ndim),ucols,ucols_new,
  ...
  for (int k=el_start; k<=el_last; k++) {
    ...
    if (comp_mat_res || comp_res || comp_mat_res_ke || comp_res_ke) {
      ...
      // Copy values from full state vector (including fictitious
...
#define WPG      (gp_data.wpg[ipg])
	...
	// implicit version - General Trapezoidal rule - parameter alpha
	  ...
	  // Galerkin - momentum
	  // resmom tiene que tener nelr*ndim
	...
	if (update_jacobian) {
	  for (int iloc=1; iloc<=nelr; iloc++) {
	    for (int jloc=1; jloc<=nelr; jloc++) {
