...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---: 
#undef __FUNC__
#define __FUNC__ "nsi_tet_les_full_ctx2::ask(char *,int &)"
int nsi_tet_les_full_ctx2::
...
#undef __FUNC__
#define __FUNC__ "nsi_tet_les_full_ctx2::assemble"
int nsi_tet_les_full_ctx2::
...
#define NN_IDX(j) ELEMIPROPS_ADD(j,0)
  ...
  FastMat2::CacheCtx2::Branchv bv2(npg),bv3(npg),bv4(npg);
  FastMat2  Hloc(&ctx,2,nel,nH),vloc_mesh(&ctx,2,nel,ndim),
    v_mesh(&ctx,1,ndim);
...
#define WAS_SET arg_data_v[ja_hmin].was_set
  ...
  nen = nel*ndof;
  FastMat2 matloc(&ctx,2,nen,nen), matlocmom(&ctx,2,nel,nel), 
    masspg(&ctx,2,nel,nel);

  FastMat2 matlocf(&ctx,4,nel,ndof,nel,ndof);
...
#define VISC (*(propel+viscosity_indx))
  ...
  FastMat2 P_supg(&ctx), W_supg(&ctx), W_supg_t(&ctx), dmatw(&ctx),
    P_pspg(&ctx,2,ndim,nel),dshapex(&ctx,2,ndim,nel);
  FastMat2 respert;

  int elem, ipg,node, jdim, kloc,lloc,ldof;

  FastMat2 Jaco(&ctx,2,ndim,ndim),iJaco(&ctx,2,ndim,ndim),
    grad_u(&ctx,2,ndim,ndim),grad_u_star(&ctx),
    strain_rate(&ctx,2,ndim,ndim),resmom(&ctx,2,nel,ndim),
    dresmom(&ctx,2,nel,ndim),matij(&ctx,2,ndof,ndof),
    Uintri(&ctx),svec(&ctx);


  FastMat2 grad_p_star(&ctx,1,ndim),u(&ctx),u_star(&ctx),du(&ctx),
    uintri(&ctx,1,ndim),rescont(&ctx,1,nel),dmatu(&ctx,1,ndim),
    ucols(&ctx),ucols_new(&ctx),
    ucols_star(&ctx),pcol_star(&ctx),pcol_new(&ctx),pcol(&ctx),
    fm_p_star(&ctx),tmp1(&ctx),tmp2(&ctx),tmp3(&ctx),
    tmp4(&ctx),tmp5(&ctx),tmp6(&ctx),massm(&ctx),tmp7(&ctx),
    tmp8(&ctx),tmp9(&ctx),tmp10(&ctx),tmp11(&ctx),tmp13(&ctx),
    tmp14(&ctx),tmp15(&ctx),dshapex_c(&ctx),xc(&ctx),
    wall_coords(&ctx,1,ndim),dist_to_wall(&ctx),tmp16(&ctx),
    tmp162(&ctx),tmp17(&ctx),tmp18(&ctx),tmp19(&ctx),
    tmp23(&ctx),tmp24(&ctx),tmp25(&ctx),
    tmp26(&ctx),tmp27(&ctx),tmp28(&ctx),tmp29(&ctx),tmp30(&ctx);

  FastMat2 tmp20(&ctx,2,nel,nel),tmp21(&ctx),vel_supg(&ctx,1,ndim);

  double pdyn; 
  FastMat2 grad_pdyn(&ctx,1,ndim); 
  FastMat2 tmp40(&ctx),tmp41(&ctx),tmp42(&ctx),
    tmp43(&ctx),tmp44(&ctx),tmp45(&ctx);

  double tmp12;
  double tsf = use_tst ? tst_factor : 0.0;

  FastMat2 eye(&ctx,2,ndim,ndim),seed(&ctx),one_nel(&ctx),
    matloc_prof(&ctx,2,nen,nen);

  FastMat2 Jaco_axi(&ctx,2,2,2),u_axi(&ctx);
