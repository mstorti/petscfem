//__INSERT_LICENSE__
//$Id: bccnsitetasm_avgvol.cpp,v 1.2.18.1 2007/02/24 02:38:51 mstorti Exp $
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
#undef __FUNC__
#define __FUNC__ "int bcconv_nsi_tet_asm_avgvol::ask(char *,int &)"
int bcconv_nsi_tet_asm_avgvol::ask(const char *jobinfo,int &skip_elemset) {
  skip_elemset = 1;
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
#undef __FUNC__
#define __FUNC__ "bcconv_nsi_tet_asm_avgvol::assemble"
int bcconv_nsi_tet_asm_avgvol::assemble(arg_data_list &arg_data_v,Nodedata *nodedata,
...
#define JDOFLOC(j,k) VEC2(jdofloc,j,k,ndof)
  ...
  FastMat2 arho_g_vp,u_star_vp,vslip_user_vp,vslip_vp,vslip_m_vp,rho_g_vp;
  ...
  FMatrix u,u_star,ucols,ucols_new,ucols_star,
...
#define WPG      (gp_data.wpg[ipg])
	...
	rho_m = compute_rho_m_2_bcc(rho_g_vp,arho_g_vp,alpha_g_vp,alpha_l,rho_l,nphases,ndim);

	compute_vel_g_2_bcc(u_star,vslip_vp,vslip_user_vp,vslip_m_vp,rho_g_vp,rho_m,
			rho_l,g_dir,d_bubble_vp,nphases,use_modified_slag_vslip,alpha_l,
			vf_star,u_star_vp,ndim);

	//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---: 

	// Residual contribution due to integration by parts of div(Vv)
	tmp2_cont.prod(normal,u_star,-1,-1);
	...
	for (int j=1; j<=nphases; j++) {
	  u_star_vp.ir(2,j);
	  res_alpha_g.ir(2,j);
	  tmp2_g.prod(normal,u_star_vp,-1,-1).scale(double(vf_star.get(j)));
	...
	/* esto quizas se pueda reusar luego al incluir TE_2
	...
	*/
	...
	for (int iloc=1; iloc<=nel; iloc++) {
	  ...
	  for (int jloc=1; jloc<=nel; jloc++) {
	    ...
	    // de continuidad Ni*div(V_v)
	    matloc.ir(1,il2).is(2,jl1,jl2-1).axpy(tmp2,WPG).rs();

	    // contribucion a las ecuaciones de la fase dispersa
	    // por la debilitacion del termino convectivo
	    for (int j=1; j<=nphases; j++) {
	      u_star_vp.ir(2,j);
	      double NiNjvn = double(tmp_g.prod(u_star_vp,tmp2,-1,-1));
	      il1 = (iloc-1)*ndof+ndim+1+j;
	      jl1 = (jloc-1)*ndof+ndim+1+j;
	      matloc.addel(-WPG*NiNjvn,il1,jl1);	      
	    }

	    /*  quizas se pueda luego reusar cuando incluyamos el TE_2
		if (coupled) {	      
