/*$Id: appalgebra.c,v 1.1.2.1 2004/03/02 17:01:29 mstorti Exp $*/
#include "appctx.h"
#include "math.h"
#include "../advdif_lin/chrono.h"
  ...
  PreLoadBegin(PETSC_TRUE,"Solver setup");  
    ...
    */
    ...
    /*
      ...
      ierr = SLESSetUp(sles,appctx->algebra.b,appctx->algebra.b);CHKERRQ(ierr);
    */
    ierr = KSPSetRhs(ksp,algebra->b);CHKERRQ(ierr);
    ierr = KSPSetSolution(ksp,algebra->x);CHKERRQ(ierr);
    ierr = KSPSetUp(ksp);CHKERRQ(ierr);

    PetscSynchronizedPrintf(comm,
			    "Solving System [proc %d]\n",myrank);
    PetscSynchronizedFlush(comm);
    PreLoadStage("Solve");  
    double t_ini=chrono1.elapsed();
    ierr = KSPSolve(ksp);CHKERRQ(ierr);CHKERRQ(ierr);
    ierr = KSPGetIterationNumber(ksp,its);CHKERRQ(ierr);
    //    ierr = SLESSolve(sles,algebra->b,algebra->x,its);CHKERRQ(ierr);
    double t_last=chrono1.elapsed()-t_ini;
    PetscSynchronizedPrintf(PETSC_COMM_WORLD,
