/*$Id: appload.c,v 1.1.2.1 2004/03/02 17:01:29 mstorti Exp $*/
#include "appctx.h"
...
int AppCtxCreate(MPI_Comm comm,AppCtx **appctx)
{
  ...
  ierr = PetscOptionsHasName(PETSC_NULL,"-save_matrix",&view->save_matrix);CHKERRQ(ierr);
  /*------------------------------------------------------------------------
...
int AppCtxDestroy(AppCtx *appctx)
{
  ...
  */
  ierr = AODataSegmentRestoreLocalIS(ao,"cell","vertex",grid->iscell,(void **)&grid->cell_vertex);CHKERRQ(ierr);
