/*$Id: appelement.c,v 1.1.2.1 2004/03/01 21:54:57 mstorti Exp $*/
#include "appctx.h"
...
int ComputeRHSElement(AppElement *phi)
{
  ...
  PetscScalar f;
  double eps_mesh;
  double xloc1=0.25,yloc1=0.75,xloc2=0.75,yloc2=0.25;
  PetscFunctionBegin;
  double pp,pp1,pp2,pp3;
  ierr = PetscOptionsGetReal(PETSC_NULL,"-eps_mesh",&eps_mesh,PETSC_NULL);CHKERRQ(ierr);
  for(i = 0; i < bn; i++){ /* loop over basis functions */
    ...
    for(j = 0; j < qn; j++){ /* loop over Gauss points */
      ...
      pp=PetscAbsReal((phi->coords[2*j]-xloc1));pp1=PetscAbsReal((phi->coords[2*j+1]-yloc1));
      pp2=PetscAbsReal((phi->coords[2*j]-xloc2));pp3=PetscAbsReal((phi->coords[2*j+1]-yloc2));
      if((pp<=eps_mesh && pp1<=eps_mesh) || (pp2<=eps_mesh && pp3<=eps_mesh)){
...
int ComputeStiffnessElement(AppElement *phi)
{
  ...
  double Pe=25.0,L=1.0,kappa=0.5;
  double u=2.0*Pe*kappa/L;
  PetscFunctionBegin;
  ...
  for(i=0;i<bn;i++){ /* loop over first basis function */
    for(j=0; j<bn; j++){ /* loop over second basis function*/
	  ...
	  (kappa*(phi->dx[i][k]*phi->dx[j][k] + phi->dy[i][k]*phi->dy[j][k])+u*((phi->weights[j]*(phi->RefVal[i][j]))*(phi->dx[i][k])+(phi->weights[j]*(phi->RefVal[i][j]))*(phi->dy[i][k])))*PetscAbsReal(phi->detDh[k]);
      }
    }
  }
  PetscFunctionReturn(0);
