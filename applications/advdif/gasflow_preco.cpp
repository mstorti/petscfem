//__INSERT_LICENSE__
//$Id: gasflow_preco.cpp,v 1.38 2005/05/26 22:07:26 mstorti Exp $
...
extern int ADVABSO_CURRENT_ELEMENT;
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::start_chunk(int &ret_options) {
  int ierr;
  FastMat2 tmp5;
  ...
  // Cut-off Mach number used in the preconditioner matrix
  GF_GETOPTDEF_ND(double,Meps,1e-6);
  // 
  GF_GETOPTDEF_ND(double,delta,1.);
  //
  GF_GETOPTDEF_ND(double,Dt,0.);
  //
  GF_GETOPTDEF_ND(int,steady,0);
  //
  GF_GETOPTDEF_ND(int,psteady,1);

  rec_Dt = (steady ? 0. : 1./Dt);
  ...
  iprecov.resize(2,ndof,ndof);
  Cpv.resize(2,ndof,ndof);
  tmp1v.resize(2,ndof,ndof);
  tmp2v.resize(2,ndof,ndof);
  tmp3v.resize(2,ndof,ndof);
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::element_hook(ElementIterator &element) {
  if (new_adv_dif_elemset) {
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
gasflow_preco_ff::gasflow_preco_ff(NewElemset *e)
  : AdvDifFFWEnth(e), old_elemset(NULL) {}

//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
gasflow_preco_ff::gasflow_preco_ff(Elemset *e)
  : old_elemset(e) {}

//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
gasflow_preco_ff::~gasflow_preco_ff() { }

//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::set_state(const FastMat2 &UU) {
  U.set(UU);
  double dummy;
  // Scalar variables
  rho = U.get(1);
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::enthalpy(FastMat2 &H) {
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::
comp_W_Gamma_N(FastMat2 &W_Ga_N,const FastMat2 &W,
	       const FastMat2 &N,double weight) {
  W_N.prod(W,N,1,2).scale(weight);
  W_Ga_N.prod(W_N,preco,1,3,2,4);
}
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::
comp_P_Gamma(FastMat2 &P_Ga,
	     const FastMat2 &P_supg) {
  P_Ga.prod(P_supg,preco,1,-1,-1,2);
}
...
compute_tau(int ijob,double &delta_sc) {
    ...
    double Peclet,velmax,psonic_speed;
    double sonic_speed = sqrt(ga*p/rho);
    double omega=ga-(ga-1.)*delta;

    iCFLc = rec_Dt*h_supg/sonic_speed;
    Mach  = velmod/sonic_speed;
    alpha = visco_supg/((velmod+tol)*h_supg);
    // alpha = 1.0;
    Mr = ((sqrt(square(Mach)+square(iCFLc)) > Meps ? sqrt(square(Mach)+square(iCFLc)) : Meps) < 1. 
	  ? (sqrt(square(Mach)+square(iCFLc)) > Meps ? sqrt(square(Mach)+square(iCFLc)) : Meps) : 1.);
    beta = (alpha*(alpha-1.)/(square(Mr)*(alpha-1+1./square(Mach+tol))) > 1. ?
	    alpha*(alpha-1.)/(square(Mr)*(alpha-1+1./square(Mach+tol))) : 1.)*square(sonic_speed);
    
    psonic_speed = sqrt(square(velmod)*square(1.+beta*square(Mr/sonic_speed)*omega)+
			4.*beta*square(Mr)*(1.-square(velmod/sonic_speed)));
   
    velmax = (psteady ? velmod+sonic_speed : 
	      .5*(velmod*(1.+beta*omega*square(Mr/sonic_speed))+psonic_speed));
    ...
    // Compute the ratio h/vel for adaptive Dt
    double dt_min_el = h_pspg/velmod;
    if (dt_min_el > 0.)
      local_dt_min = (local_dt_min > dt_min_el ? dt_min_el : local_dt_min);

}

//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::compute_flux(const FastMat2 &U,
  ...
  //---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
  // Preconditioner matrix for pseudo-time strategy
  // following Choi & Merkle work - Primitive variables
  double sonic_speed = sqrt(ga*p/rho);
  double he = 0.;
  ...
  if (advdf_e) {
    ...
    if(velmod>tol) {
      FastMat2::choose(0);
      // svec:= a streamline oriented unit vector
      svec.set(vel).scale(1./velmod);
      he = tmp9.prod(grad_N,svec,-1,1,-1).sum_abs_all();
      he = (he < tol ? tol : he);
      he = 2./he;
    } else {
    ...
    FastMat2::leave();
  } else if (ADVABSO_CURRENT_ELEMENT!=INT_MAX) {
    advabso_e = dynamic_cast<const AdvectiveAbso *>(elemset);
    assert(advabso_e);
    he = advabso_e->h_ele.e(ADVABSO_CURRENT_ELEMENT);
  } else
    throw GenericError("Current element in abso bc's.");

  iCFLc = rec_Dt*he/sonic_speed;
  Mach  = velmod/sonic_speed;
  alpha = visco/(rho*(velmod+tol)*he);
  // alpha = 1.0;
  Mr = ((sqrt(square(Mach)+square(iCFLc)) > Meps ? sqrt(square(Mach)+square(iCFLc)) : Meps) < 1. 
	? (sqrt(square(Mach)+square(iCFLc)) > Meps ? sqrt(square(Mach)+square(iCFLc)) : Meps) : 1.);
  beta = (alpha*(alpha-1.)/(square(Mr)*(alpha-1+1./square(Mach+tol))) > 1. ?
  ...
  preco.set(0.);
  preco.setel(1./(beta*square(Mr)),1,ndof);
  preco.is(1,vl_indx,vl_indxe).is(2,vl_indx,vl_indxe).eye(rho).rs();
  preco.is(1,vl_indx,vl_indxe).ir(2,ndof).set(vel).scale(1./(beta*square(Mr))).rs();
  preco.ir(1,ndof).is(2,vl_indx,vl_indxe).set(vel).scale(rho).rs();
  preco.setel(-ga/g1*p/rho,ndof,1);
  preco.setel(ga/g1+(ga/g1*p/rho+.5*square(velmod))/(beta*square(Mr))-delta,ndof,ndof);

  //---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
  // Inverse preconditioner matrix for pseudo-time strategy
  // in viscous basis
  iprecov.set(0.);
  iprecov.setel(beta*square(Mr),1,1);
  iprecov.is(1,vl_indx,vl_indxe).is(2,vl_indx,vl_indxe).eye(1./rho).rs();
  iprecov.is(1,vl_indx,vl_indxe).ir(2,1).set(vel).scale(-1./rho).rs();
  iprecov.ir(1,ndof).is(2,vl_indx,vl_indxe).set(vel).scale(-g1/(ga*Rgas*rho)).rs();
  iprecov.setel(-p/(Rgas*rho*rho)+ 
		g1/(ga*Rgas*rho)*(.5*square(velmod)+delta*beta*square(Mr)),ndof,1);
  iprecov.setel(g1/(ga*Rgas*rho),ndof,ndof);

  //---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
  // 
  Cpv.set(0.);
  Cpv.setel(rho/p,1,1);
  Cpv.setel(-Rgas*rho*rho/p,1,ndof);
  Cpv.is(1,vl_indx,vl_indxe).is(2,vl_indx,vl_indxe).eye(rho).rs();
  Cpv.is(1,vl_indx,vl_indxe).ir(2,1).set(vel).scale(rho/p).rs();
  Cpv.ir(1,ndof).is(2,vl_indx,vl_indxe).set(vel).scale(rho).rs();
  Cpv.setel(1/g1+.5*rho*square(velmod)/p,ndof,1);
  Cpv.setel(-.5*Rgas*square(rho*velmod)/p,ndof,ndof);
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::comp_A_jac_n(FastMat2 &A_jac_n, FastMat2 &normal) {
  tmp1v.prod(Cpv,iprecov,1,-1,-1,2);
  Ajac_tmp.prod(tmp1v,Ajac,2,-1,1,-1,3);
  A_jac_n.prod(Ajac_tmp,normal,-1,1,2,-1);
}

//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::comp_A_grad_N(FastMat2 & A_grad_N,FastMat2 & grad_N) {
  A_grad_N.prod(Ajac,grad_N,-1,2,3,-1,1);
}

//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::comp_grad_N_D_grad_N(FastMat2 &grad_N_D_grad_N,
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
int gasflow_preco_ff::comp_grad_N_dDdU_N(FastMat2 &grad_N_dDdU_N, FastMat2 &grad_U,
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::comp_N_N_C(FastMat2 &N_N_C,FastMat2 &N,double w) {
  tmp2.prod(N,N,1,2).scale(w);
  N_N_C.prod(tmp2,Cjac,1,3,2,4);
}

//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::comp_N_P_C(FastMat2 &N_P_C, FastMat2 &P_supg,
			   FastMat2 &N,double w) {
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::comp_P_supg(FastMat2 &P_supg) {
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::compute_rec_h_rgn(FastMat2 &r_dir) {
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff::compute_rec_h_rgn(FastMat2 &r_dir,FastMat2 &r_dir_aux) {
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff:: compute_tau_switched(double &tau_con, double &tau_mom, 
				       double &tau_ene) {
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:
void gasflow_preco_ff:: compute_shocap(double &delta_sc) {
