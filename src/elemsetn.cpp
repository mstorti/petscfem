...
//#define PSTAT(iele,j,k) VEC3(pstat,iele,j,nel,k,ndof)
#define RETVAL(iele,j,k) VEC3(retval,iele,j,nel,k,ndof)
//#define RETVALT(iele,j) VEC2(retval,iele,j,ndoft)
//#define REFREST(iele,j) VEC2(refres,iele,j,ndoft)
#define RETVALMAT(iele,j,k,p,q) VEC5(retval,iele,j,nel,k,ndof,p,nel,q,ndof)
#define ICONE(j,k) VEC2(icone,j,k,nel)

int assemblen(Mesh *mesh,ArgList argl,
	      Dofmap *dofmap,char *jobinfo,
...
#define ARGVJ (arg_data_v[j])
  ...
  for (int ielset=0; ielset<da_length(elemsetlist); ielset++) {
    ...
    for (j=0; j<narg; j++) 
      argl[j]->chunk_pre_operations(chunk_size,ndoft);
