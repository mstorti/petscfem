...
#if 0
#undef __FUNC__
#define __FUNC__ "chunkpos" 
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---: 
void chunkpos(int iord, int size, int *pos, int *chunk) {
    *chunk = (iord-1)/size;
    *pos = ((iord-1) % size)+1;
}
#endif
...
void test_vector_resize(void) {
  int Nvec=10;
  int Nelems=1000000;
  vector < vector < double > > header(Nvec);
  for (int j=0; j<Nvec; j++) {
    printf("filling vector %d\n",j);
    for (int k=0; k<Nelems; k++) {
      header[j].push_back(double(k));
    }
    VOID_IT(header[j]);
  }
  // wait_from_console("exiting test_vector_resize()"); 
}
#endif
...
int read_mesh(Mesh *& mesh,char *fcase,Dofmap *& dofmap,
  ...
  while (1) {
	...
	PetscPrintf(PETSC_COMM_WORLD,
		    "No name for this element!! setting as \"anonymous\"...\n");
