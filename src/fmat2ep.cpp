...
//__INSERT_LICENSE__
//$Id: fmat2ep.cpp,v 1.21.62.2 2005/03/27 22:06:45 mstorti Exp $
#include <math.h>
#include <stdio.h>
...
//---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---:---<*>---: 
struct fastmat2_stat {
  double non_null;
  double proc;
  double times;
  fastmat2_stat() :
    non_null(0), proc(0), times(0) { }
};

map<string,fastmat2_stat> fastmat2_stat_map;
string fastmat2_stat_current_string = "";

void fastmat_prod_stat() {
#ifdef DO_FM2_STAT
  map<string,fastmat2_stat>::iterator 
    q = fastmat2_stat_map.begin();
  double total = 0, total_nn=0, total_times=0;
  while (q!=fastmat2_stat_map.end()) {
    total += q->second.proc;
    total_nn += q->second.non_null;
    total_times += q->second.times;
    q++;
  }

  q = fastmat2_stat_map.begin();
  printf("--- FastMat2 prod statistics \n");
  while (q!=fastmat2_stat_map.end()) {
    fastmat2_stat &stat = q->second;
    printf("key %12s, times %10g, proc %10.4g (%5.1f%%), "
	   "non_null %10.4g (%5.1f%%)\n",
	   q->first.c_str(), 
	   stat.times,
	   stat.proc,
	   stat.proc/total*100.0,
	   stat.non_null,
	   stat.non_null/stat.proc*100.0);
    stat.times = 0;
    stat.proc = 0;
    stat.non_null = 0;
    q++;
  }
  printf("- Total times %g, proc %g, non_null %g, \n",
	 total_times,total,total_nn);
#endif
}
...
struct prod_cache : public FastMatSubCache {
  ...
  /// Dtor
  ~prod_cache() {
    // printf("at `~prod_cache()': fastmat2_stat_current_string \"%s\"\n",
    // fastmat2_stat_current_string.c_str());
    if (key!="") {
      fastmat2_stat &stat = fastmat2_stat_map[key];
      stat.proc += proc;
      stat.non_null += non_null;
      stat.times += times;
    }
  }
};
...
FastMat2 & FastMat2::prod(const FastMat2 & A,const FastMat2 & B,
  ...
  if (!was_cached  ) {
    ...
    assert(!cache->sc);
    cache->sc = pc;
    pc->key = fastmat2_stat_current_string;
